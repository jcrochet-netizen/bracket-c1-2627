 ...
 * Ligue des champions (art. 19.02 à 19.04).
 *
 *   node verify-annexe-b.js      # code de sortie 0 si tout est conforme
 *
 * La table `ANNEXE_B` est lue directement dans bracket-c1.html : le contrôle
...
const src = fs.readFileSync(path.join(__dirname, "bracket-c1.html"), "utf8");
const bloc = src.match(/var ANNEXE_B = \[([\s\S]*?)\];/);
if (!bloc) { console.error("✗ ANNEXE_B introuvable dans bracket-c1.html"); process.exit(1); }
const A = eval("[" + bloc[1] + "]");
